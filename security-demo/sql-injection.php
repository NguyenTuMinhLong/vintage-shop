...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        label { display: block; margin-bottom: 5px; font-weight: 500; color: #8b949e; }
...
<body>
    <div class="container">
        <header>
            ...
            <span class="badge">Bảo trì: SQL Injection</span>
        </header>
        ...
        <section class="section">
            ...
            <div class="info-box">
                <strong>Tài khoản mẫu thật:</strong> <code>admin@example.com / admin123</code>
            </div>
        ...
        <div class="demo-grid">
            ...
            <div class="section danger">
                ...
                <form method="post" action="?vuln=1">
                    <div class="form-group">
                        ...
                        <input type="email" name="email" id="vuln_email" placeholder="admin@example.com" 
                               value="<?php echo isset($_POST['email']) && isset($_POST['vuln']) ? htmlspecialchars($_POST['email']) : ''; ?>">
                    </div>
                    <div class="form-group">
                ...
                <div class="sql-query">
                    <span>SELECT</span> full_name, email, role <span>FROM</span> demo_users 
                    <span>WHERE</span> email = <span>'[input]'</span> <span>AND</span> password = <span>'[input]'</span> <span>LIMIT 1</span>
                </div>
            ...
            <div class="section safe">
                ...
                <form method="post" action="?safe=1">
                    <div class="form-group">
                        ...
                        <input type="email" name="email" id="safe_email" placeholder="admin@example.com"
                               value="<?php echo isset($_POST['email']) && isset($_POST['safe']) ? htmlspecialchars($_POST['email']) : ''; ?>">
                    </div>
                    <div class="form-group">
                ...
                <div class="sql-query">
                    <span>SELECT</span> full_name, email, role <span>FROM</span> demo_users 
                    <span>WHERE</span> email = <span>?</span> <span>AND</span> password = <span>?</span> <span>LIMIT 1</span>
                </div>
    ...
    if (isset($_POST['vuln'])) {
        ...
        foreach ($users as $user) {
            // Check if email matches (simulating the vulnerability)
            $email_pattern = str_replace("'", "", $email); // Remove quotes for simulation
            if ($user['email'] === $email || strpos($email, "'") !== false) {
                // If SQL injection detected, always allow login
                if ($email === "admin@example.com' -- " || strpos($email, "' --") !== false) {
                    $found = true;
                    $vuln_success = true;
                    $vuln_message = "ĐĂNG NHẬP THÀNH CÔNG! Payload SQL Injection đã hoạt động!<br>Chào <strong>{$user['full_name']}</strong> (Role: {$user['role']})<br><br>Câu truy vấn thực tế:<br><code>SELECT * FROM users WHERE email = '{$email}' AND password = '{$password}'</code><br><br>⚠️ Ký tự <code>--</code> biến phần mật khẩu thành comment, không cần mật khẩu đúng!";
                    break;
                }
                // Normal login check
